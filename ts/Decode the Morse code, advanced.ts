...
//     transmit at different speed. An amateur person may need a few seconds to transmit a single character, a skilled
//     professional can transmit 60 words per minute, and robotic transmitters may go way faster.
//
...
//     is not connected (remote key is up), 0 is recorded. After the message is fully received,
...
//     All the test strings would be valid to the point that they could be reliably decoded as described above, so you
//     may skip checking for errors and exceptions, just do your best in figuring out what the message is!
//
// Good luck!
//
...
const detectUnitSize = (stream: string) => {
    ...
    return Math.min(...transitions);
}

const reduceMorseStream = (stream: string) => {
    stream = stream.replace(/^0+|0+$/g, '');

    const unitSize = detectUnitSize(stream);
    let reducedStream = "";

    for (let i = 0; i < stream.length; i += unitSize) {
        let chunk = stream.slice(i, i + unitSize);

        if (chunk.length < unitSize) break;

        let onesCount = chunk.split("").filter(bit => bit === "1").length;
        let zerosCount = unitSize - onesCount;

        reducedStream += onesCount > zerosCount ? "1" : "0";
    }
